...
export const getProtocolOwner = async (): Promise<[PublicKey, number]> => {
	return await PublicKey.findProgramAddress([], program.programId);
};

export const getLPMintPDA = async (
...
export const getMpMetadataPDA = async (
...
export const getLPTokenAccountPDA = async (
	poolPDA: PublicKey,
	vaultPDA: PublicKey,
	tickBN: anchor.BN,
...
export const depositLiquidity = async (
	...
	// Liquidity Pool PDA
	const poolPDA = await getPoolPDA(protocolToInsure);
	try {
	...
	// Get tick account
	const tickAccountPDA = await createTickAccount(
		poolPDA,
		tokenMint,
		tick,
		liquidityProvider
	...
	const nftAccount = await getLPTokenAccountPDA(
		poolPDA,
		vaultPDA,
		tickBN,
		nextTickPositionBN
	...
	// Get bitmap
	const bitmapPDA = await getLiquidityPositionBitmapPDA(poolPDA, tokenMint);
	try {
...
export const estimateYearlyPremium = async (
	...
	let bitmapPDA = await getLiquidityPositionBitmapPDA(pool, tokenMint);
	...
	// Get tick account
	let tickAccountPDA = await getTickAccountPDA(pool, tokenMint, tick);
	...
	while (amountToCover.gt(new anchor.BN(0)) && tick !== -1) {
		...
		bitmapPDA = await getLiquidityPositionBitmapPDA(pool, tokenMint);
...
export const buyInsurance = async (
	...
	if (newInsuredAmountBN.gte(insuredAmount)) {
		...
		const liquidityPositionsPDA = await getLiquidityPositionBitmapPDA(
			pool,
			tokenMint
		);
