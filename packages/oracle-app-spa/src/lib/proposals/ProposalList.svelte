<script lang="ts">
	...
	import { globalStore, loadingState } from '../../store/index';
	// Input search
	...
	function filterProposals(search: string) {
		if (search.length > 2)
			filteredProposals = proposals.filter(
				(val) =>
					val.account.name.includes(search) ||
					val.account.description.includes(search)
			);
		else {
				...
				>
					...
					>
						<div
							...
							`}
						>
							<p class="p p--white p--medium p--margin-0 ">
